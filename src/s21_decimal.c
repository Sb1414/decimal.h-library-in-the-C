...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
    ...
    int return_value = OK, sign1 = s21_get_sign(&value_1), sign2 = s21_get_sign(&value_2);
    unsigned int mask = 1u << 31;
    if (!sign1 && !sign2) {
        int scale_1 = (char)(value_1.bits[3] >> 16), scale_2 = (char)(value_2.bits[3] >> 16);
        if (scale_1 != scale_2) {
...
void s21_div_only_bits(s21_decimal value_1, s21_decimal value_2, s21_decimal *buf,
                   s21_decimal *result) {
    ...
    for (int i = s21_last_bit(value_1); i >= 0; i--) {
        ...
        if (s21_is_greater_or_equal(*buf, value_2) == TRUE) {
            ...
            s21_offset_left(result, 1);
            if (i != 0) s21_offset_left(buf, 1);
            if ((i - 1) >= 0 && s21_get_bit(value_1, i - 1)) s21_set_bit(buf, 0, 1);
        }
    }
}
...
int s21_shift_left(s21_decimal *dec, int shift) {
    ...
    if (highest_bit + shift > 95) {
        ...
        for (int i = 0; i < shift; i++) {
            ...
            if (last_low_bit_set)
                s21_set_bit(dec, 32, 1);
            if (last_mid_bit_set)
                s21_set_bit(dec, 64, 1);
        }
    }
    if (s21_get_bit(*dec, 127) && ret_val)
        ret_val = 2;
    return ret_val;
}
